...
namespace JoblyWebApi.Services
{
    public class ApplyService : IApplyService
    {
        ...
        public ApplyService(IUserRepository userRepository )
        {
            _userRepository = userRepository;
        }

        public async Task<bool> ExecuteApply(NaukriUser user)
        {
            ...
            try
            {
                ...
                wait.Until(ExpectedConditions.ElementIsVisible(By.Id("usernameField"))).SendKeys(user.NaukriId);
                ConfigManager.Delay5();
                wait.Until(ExpectedConditions.ElementIsVisible(By.Id("passwordField"))).SendKeys(user.NaukriPassword);
                ConfigManager.Delay5();
                    ...
                    try
                    {
                        ...
                        if (tab != null)
                        {
                            ...
                            for (int i = 0; i < jobs.Count; i++)
                            {
                                ...
                                await ApplyJob(job, driver, wait, user.UserId);
        ...
        public Task<NaukriUser> GetNaukriUser(int userId)
        {
            return _userRepository.GetNaukriUser(userId);
        }

        private async Task ApplyJob(IWebElement job, ChromeDriver driver, WebDriverWait wait, int userId)
        {
            try
            {
